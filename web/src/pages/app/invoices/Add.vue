...
const invoice = ref({ details: [{ description: 'DDD'}] } as Invoice);
        ...
        <div class="flex justify-between">
          <div class="flex">
            <div class="w-[120px] font-bold">
              ...
              <div class="w-[50px]"><PrimaryButton @click="showSelectPatientPopup">Pick</PrimaryButton></div>
            </div>
            <div class="space-y-2">
              <div class="w-[300px]"><TextField placeholder="Patient Name" v-model="invoice.name" required /></div>
              <div><TextField placeholder="Billing Address" v-model="invoice.billing_address" required /></div>
            </div>
          </div>
          <div class="">
            <div class="space-y-2 flex flex-col">
              <div class="flex items-center">
                ...
                <div class="w-[250px] ml-3"><TextField placeholder="Invoice number" v-model="invoice.invoice_number" required></TextField></div>
              </div>
              <div class="flex items-center justify-between">
                <div class="font-bold w-[120px] text-right">Invoice Date:</div>
                <div class="w-[250px] ml-3"><TextField placeholder="Invoice number" v-model="invoice.invoice_date" type="date" required></TextField></div>
              </div>
              <div class="flex items-center justify-between">
                <div class="font-bold w-[120px] text-right">Due Date:</div>
                <div class="w-[250px] ml-3"><TextField placeholder="Invoice number" v-model="invoice.due_date"  type="date" required></TextField></div>
              </div>
            </div>
          </div>
        </div>
        <table class="w-full my-5">
          ...
          <tbody>
            <tr v-for="d, key in invoiceDetails" :key="key">
              <td><TextField class="w-full" v-model.number="d.description"/></td>
              <td><TextField class="w-[120px]" v-model.number="d.qty" type="number" /></td>
              <td><TextField class="w-[120px]" v-model="d.unit" /></td>
              <td><TextField class="w-[120px]" v-model.number="d.amount" type="number" /></td>
              <td class="text-right px-2">{{ ((d.qty || 0) * (d.price || 0)).toLocaleString("en-US") }}</td>
              <td class=" w-[50px]">
                <button title="Delete item" @click="() => removeRow(`${key}`)" class="rounded border-[1px] w-[50px] border-blue-600 py-1 hover:bg-blue-600 hover:text-white active:bg-blue-700 flex items-center justify-center text-center">  
            ...
            <tr>
              <td colspan="6">
                <div class="text-right">
                  <ActionButton @click="addRow" :icon-src="PlusIcon">Add Row</ActionButton>
                </div>
              </td>
            </tr>
          </tfoot>
